...
from phone9p import phone_in_case
...
cf = (98.5-(2.70-DX), 128.5-(2.70-DX), -44.5, 5.5)
...
# ============================================== 6. NEW POCKET + LIP ==========
pock = rrect_cs(POCK_X0, POCK_X1, POCK_Y0, POCK_Y1, POCK_R) + \
       m3.CrossSection.square([POCK_R+1.0, POCK_Y1-POCK_Y0]).translate([POCK_X1-POCK_R-1.0, POCK_Y0])
...
for y0, y1 in [(-59.20, -56.25), (14.97-0.75, 17.20)]:
    C -= box(POCK_X0-0.01, -13.10-DX, y0, y1, 3.00, 7.00)       # bottom-cap detent pockets
for y0, y1 in [(-58.60, -56.65), (15.40-0.75, 16.60)]:
    C -= box(136.90-(2.70-DX), POCK_X1+0.01, y0, y1, 1.05, 8.95) # top-cap rail tips
stats(C, "6 pocket")
...
# ============================== 8. CAMERA: SHELF + OPENING + SLIDE CHANNEL ====
C += rrect_taper(SHELF_X0+SHELF_TAPER, CASE_X1-SHELF_TAPER,
                 SHELF_Y0+SHELF_TAPER, SHELF_Y1-SHELF_TAPER, 6.0,
                 SHELF_Z, Z_BACK_WIN, SHELF_TAPER)
C -= box(CHAN_X0, ENTRY_X1, CO_Y0, CO_Y1, CHAN_Z, POCK_Z0+0.01)
C -= rrect(CO_X0, CO_X1, CO_Y0, CO_Y1, CO_R, -12.0, POCK_Z0+0.05); stats(C, "8 camera")
...
C -= port(SPK_Y0-1.50, SPK_Y1+1.50, SPK_Z0-0.80, SPK_Z1+0.80, 1.30)   # speaker
C -= port(-21.02-7.50, -21.02+7.50, 0.05, 6.10, 1.50)                 # USB-C (generous)
C -= port(MIC_Y0-1.40, MIC_Y1+1.40, MIC_Z0-1.10, MIC_Z1+1.10, 1.10)   # mic
C -= port(AUX_Y0-1.40, AUX_Y1+1.40, AUX_Z0-1.10, AUX_Z1+1.10, 1.10)   # 2nd port hole
stats(C, "10 ports")
...
out.export(os.path.join(OUT, "case_pixel9pro.stl"))
phone_in_case().export(os.path.join(OUT, "_phone9pro_in_case.stl"))
print("wrote", os.path.join(OUT, "case_pixel9pro.stl"))
