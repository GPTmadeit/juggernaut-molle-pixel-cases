...
CAP_HOLES = (slot_row(SPK_Y0-0.6, SPK_Y1+0.6, 3, SPK_Z0-0.55, SPK_Z1+0.55) +
             slot_row(LFT_Y0-0.6, LFT_Y1+0.6, 3, LFT_Z0-0.55, LFT_Z1+0.55))
...
tc -= extrude_x(rrect_cs(TOPMIC_Y0-1.0, TOPMIC_Y1+1.0, TOPMIC_Z0-1.0, TOPMIC_Z1+1.0, 0.8),
                119.32+DX-0.5, 139.90+DX+0.5)          # the 6a's top mic sits between the stock vents
save(tc, "top_cap_pixel6a.stl")
...
case = to_mf(trimesh.load(os.path.join(OUT,"case_pixel6a.stl")))
grown = case
...
save(cap, "camera_protector_cap_pixel6a.stl")
...
save(insert((VOL_X0-REC_MX, VOL_X1+REC_MX),
            ...
            thin=(VOL_MID-1.1, VOL_MID+1.1)), "volume_button_pixel6a.stl")
save(insert((PWR_X0-REC_MX, PWR_X1+REC_MX), [(PWR_X0-WIN_MX, PWR_X1+WIN_MX)]), "power_button_pixel6a.stl")
print("done")
